...
/** Returns canvas only (for 2D preview). Same drawing as createAppleTexture. */
export function createAppleTextureCanvas() {
    const canvas = createCanvas();
    const ctx = canvas.getContext('2d');
    const SIZE = 256;
    const baseGrad = ctx.createLinearGradient(0, 0, 0, SIZE);
    ...
    for (let i = 0; i < 35; i++) {
        ...
        const r = 18 + Math.random() * 25;
        const shade = Math.random();
        ctx.fillStyle = shade < 0.4 ? 'rgba(100, 20, 20, 0.5)' : shade < 0.7 ? 'rgba(160, 35, 35, 0.4)' : 'rgba(220, 80, 80, 0.3)';
        ctx.beginPath();
        ctx.arc(x, y, r, 0, Math.PI * 2);
        ctx.fill();
    }
